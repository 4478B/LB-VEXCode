...
void handleMacro()
{
  if (Controller1.ButtonX.pressing())
  {
    ...
    if (!macro)
      count = 0; // Reset count when turning off
  ...
  {
    if (count >= 150)
    { // After ~0.5s of reversing
      macro = false;
      count = 0;
    }
    else if (count >= 4)
    { // After 80ms of ring detection
...
void handleArm()
{
  ...
  else if ((Controller1.ButtonLeft.pressing() || Controller2.ButtonLeft.pressing()) && pidRunning == false && armPos != 3)
  {
    ...
    if (pidThread.joinable())
      pidThread.join(); // Ensure the previous thread is finished
    pidThread = vex::thread(setArmTop);
    // tune according to motor encoder values, could be negative idk
  }

  if (pidRunning == true && pidThread.joinable())
  {
    pidThread.join();
    pidRunning = false;
  }
